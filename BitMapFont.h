...
public:
	...
	void textDrawRandom(int x, int y, float size, char* string, int maxRand, int countDown);           // ランダム文字表示

	void setFontShadowOffset(int ofsX, int ofsY);                                          // 影フォント位置オフセット
	void textDraw(int x, int y, float size, const char* string);                                       // テキストを描画
	void textDrawWithShadow(int x, int y, float size, char* string);                                   // 影付きでテキスト描画

	void textDrawMessage(int x, int y, float size, char* string, unsigned int display_lengh);          // メッセージ風表示（文字数制限あり）
	void textDrawMessageWithShadow(int x, int y, float size, char*string, unsigned int display_lengh); // メッセージ風表示（文字数制限あり）
