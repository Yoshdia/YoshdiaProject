...
void BitmapText::textDraw(int x, int y, float size, const char * string)
{
	//フォント使用可能か
	if (!m_useflag)
		return;
	...
	while (*string)
	{
		...
		if (img >= 0)
		{
			DrawRotaGraph(text_x,text_y,size,0,m_fontImg[img],m_bgalpha);
		}

		// 文字描画位置進める
		text_x += (int)(m_sizeX*(size/1.3));
		string++;
	}
...
void BitmapText::textDrawWithShadow(int x, int y, float size, char * string)
{
	//影文字描画
	SetDrawBright(m_shadow_R, m_shadow_G, m_shadow_B);
	textDraw(x + m_shadowOffsetX, y + m_shadowOffsetY,size, string);

	//文字描画
	SetDrawBright(R, G, B);
	textDraw(x, y, size, string);

}

void BitmapText::textDrawRandom(int x, int y, float size, char* string, int maxRand, int countDown)
{
	...
	textDraw(x, y, size, randString);

	delete[] randString;
}

void BitmapText::textDrawMessage(int x, int y, float size, char * string, unsigned int display_lengh)
{
	...
	textDraw(x, y, size, tmpStr);

	delete[] tmpStr;
}

void BitmapText::textDrawMessageWithShadow(int x, int y, float size, char * string, unsigned int display_lengh)
{
	// 文字数をカウントし、表示文字数が本来の文字数より多ければそのまま表示
	size_t len = strlen(string);
	if (len < display_lengh)
	{
		textDrawWithShadow(x, y, size, string);
		return;
	}
	...
	textDrawWithShadow(x, y, size, tmpStr);
